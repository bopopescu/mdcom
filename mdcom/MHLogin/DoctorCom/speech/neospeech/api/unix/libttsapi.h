...
//DllExport int TTSRequestFileEx_SSML(char *szServer, int nPort, char *pText, int nTextLen, char *szSaveDir, char *szSaveFile, int nSpeakerID, int nVoiceFormat, int nVolume, int nSpeed, int nPitch, int nDictIndex, int *pMarkSize, TTSMARK** ppTTSMark, int *pVoiceLen);
