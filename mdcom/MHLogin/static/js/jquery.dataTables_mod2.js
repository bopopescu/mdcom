...
(function($) {
	...
	_oExt.oStdClasses = {
		...
		"sPageButton": "paginate_button ui-corner-all",
		...
		"sPageNext": "next",
		"sPageLast": "last",
		"sLetterPaginationButton": "letter_pagination_button",
	...
	_oExt.oPagination = {
		...
		/*
		 * Variable: letter_pagination
		 * Purpose:  Letter pagination
	 	 * Scope:    jQuery.fn.dataTableExt.oPagination
		 */
		"letter_pagination": {
			/*
	     ...
	     *           	 node 		: nPaging 			- the DIV which contains this pagination control
			...
			{	
				...
				var oClasses = oSettings.oClasses;
				var sImageButtonClass = oClasses.sLetterPaginationButton;
				nFirst.className = oClasses.sPageButton+" "+oClasses.sPageFirst+" "+sImageButtonClass;
				nPrevious.className = oClasses.sPageButton+" "+oClasses.sPagePrevious+" "+sImageButtonClass;
				nNext.className= oClasses.sPageButton+" "+oClasses.sPageNext+" "+sImageButtonClass;
				nLast.className = oClasses.sPageButton+" "+oClasses.sPageLast+" "+sImageButtonClass;
			...
			/*
			 * Function: oPagination.letter_pagination.fnUpdate
			 * Purpose:  Update the list of page buttons shows
			 * Returns:  -
	 		 * Inputs:   object		:	oSettings 			- dataTables settings object
			 *           function	:	fnCallbackDraw 		- draw function to call on page change
			...
			"fnUpdate": function ( oSettings, fnCallbackDraw )
			{
				...
				var iPages = Math.ceil((oSettings.fnRecordsDisplay()) / oSettings._iDisplayLength);
				var iPageCount = (oSettings.iLetterPaginationShowPages > iPages) ? iPages : oSettings.iLetterPaginationShowPages;
				var iPageCountHalf = Math.ceil(iPageCount / 2);
				var iCurrentPage = Math.ceil(oSettings._iDisplayStart / oSettings._iDisplayLength) + 1;
				var sList = "";
				var sPageStartLong = "", sPageEndLong = "";
				var sPageStartShort = "", sPageEndShort = "";
				var sLetterA = "", sLetterB = "";
				var sPageClass = "";
				var i, iPageOffset, iCountItems, iFlag;
				var oClasses = oSettings.oClasses;
				...
				if(iCurrentPage<iPageCountHalf)
				{
					iFrontIndex = iCurrentPage;
				}
				else
				{
					if(iPages-iCurrentPage<iPageCount-iPageCountHalf)
					{
						iFrontIndex = iPageCount-(iPages-iCurrentPage);
					}
					else
					{
						iFrontIndex = iPageCountHalf;
					}
				}
				
				iPageOffset = 0-(iFrontIndex-1)*oSettings._iDisplayLength;
				var iBuildIndex = 0;
				var iLetterRange;
				
				// Building pagination
				function _getRowVal(iOffset)
				{
					var olIndex = oSettings.aiDisplay[oSettings._iDisplayStart+iOffset];
					var tableRow = oSettings.aoData[olIndex]._aData;
					return tableRow[oSettings.iColumnToSort];	
				}
				...
				while(iPageCount--)
				{
					...
					sPageClass = "step";
					if(iBuildIndex==iFrontIndex)
					{
						sPageClass +=  " "+oClasses.sPageButtonActive;
					}
					else
					{
						sPageClass += ( iBuildIndex<iFrontIndex ) 
									   ? "-"+( iFrontIndex-iBuildIndex ).toString()
									   : "+"+( iBuildIndex-iFrontIndex ).toString();
					}
					
					// Building PageStart/PageEnd strings
					sPageStartLong 	= _getRowVal(iPageOffset);
					
					i = oSettings.fnRecordsDisplay() - (oSettings._iDisplayStart+iPageOffset);
					iCountItems = ( i>=oSettings._iDisplayLength )
									? ( oSettings._iDisplayLength )
									: ( i%oSettings._iDisplayLength );
					sPageEndLong 	= _getRowVal(iPageOffset + iCountItems - 1);
					
					iLetterRange = (sPageStartLong.length > sPageEndLong.length)
									? (sPageStartLong.length)
									: 2;//(sPageEndLong.length);
					iFlag = 0;
					sPageStartShort = "";
					sPageEndShort = "";
					//delete <span class="warpName"> and </span> for showing correctly
					var re = /<span class=\"warpName\">/i;
					if($.browser.msie && (parseInt($.browser.version) < 9)){
						re = /<span class=\warpName\>/i;
					}
					var re2 = /<\/span>/i;
					sPageStartLong = sPageStartLong.replace(re,'');
					sPageStartLong = sPageStartLong.replace(re2,'');
					sPageEndLong = sPageEndLong.replace(re,'');
					sPageEndLong = sPageEndLong.replace(re2,'');
					//add trim function for deleting space at the front and end of string.
					sPageStartLong = jQuery.trim(sPageStartLong);
					sPageEndLong = jQuery.trim(sPageEndLong);
					
					for( i=0 ; i<iLetterRange && !iFlag ; i++ )
					{
						sLetterA = sPageStartLong.charAt(i).toLowerCase();
						sLetterB = sPageEndLong.charAt(i).toLowerCase();
						
						sPageStartShort += sPageStartLong.charAt(i);
						sPageEndShort += sPageEndLong.charAt(i);
						
						if(i>0 && sPageStartShort.toLowerCase() != sPageEndShort.toLowerCase())
						{
							iFlag = 1;
						}
					}
					...
					sList += '<span class="'+oClasses.sPageButton+' '+sPageClass+'">'+sPageStartShort+'-'+sPageEndShort+'</span>';
					
					iPageOffset += oSettings._iDisplayLength;
				}
				...
				var fnClick = function() {
					// Use the information in the element to jump to the required page 
					my_sClasses = $(this).attr('class');
					my_aClasses = my_sClasses.split(" ");

					for( i=0 ; i < my_aClasses.length ; i++ )
					{
						my_sClass = my_aClasses[i];

						if(my_sClass.substr(0,4)=="step" && my_sClass.length>4){

							if(my_sClass.charAt(4)=="-")
							{
								n = my_sClass.charAt(5);
								while(n--){
									oSettings._iDisplayStart -= oSettings._iDisplayLength;
								}
							}
							else if(my_sClass.charAt(4)=="+")
							{
								n = my_sClass.charAt(5);
								while(n--){
									oSettings._iDisplayStart += oSettings._iDisplayLength;
	...
	{		
		...
		function classSettings ()
		{
			 ...
			 */
			this.oLanguage = {
				"sProcessing": MESSAGE.JQUERY_DATATABLES_PROCESSING,
				"sLengthMenu": MESSAGE.JQUERY_DATATABLES_SLENGTHMENU,
				"sZeroRecords": MESSAGE.JQUERY_DATATABLES_SZERORECORDS,
				"sInfo": MESSAGE.JQUERY_DATATABLES_SINFO,
				"sInfoEmpty": MESSAGE.JQUERY_DATATABLES_SINFOEMPTY,
				"sInfoFiltered": MESSAGE.JQUERY_DATATABLES_SINFOFILTERED,
				"sInfoPostFix": "",
			...
			/*
			 * Variable: iColumnToSort
			 * Purpose:  Note which is the column to do alphabeticall sorting on
			 * Scope:    jQuery.dataTable.classSettings
			 */
			this.iColumnToSort = null;
			
			/*
			 * Variable: iColumnToSort
			 * Purpose:  Note which is the secondary column to do alphabeticall sorting on
			 * Scope:    jQuery.dataTable.classSettings
			 */
			this.iColumnToSortSecondary = null;
			
			/*
			 * Variable: iLetterPaginationShowPages
			 * Purpose:  Note how many pagination buttons in between next and prev
			 * Scope:    jQuery.dataTable.classSettings
			 */
			this.iLetterPaginationShowPages = 3;
		...
		this.fnFilter = function( sInput, iColumn, bEscapeRegex )
		{
			...
			// add by Mingbo.Wang 04/25/2012
			sInput = $.trim(sInput);
			if (oSettings.iMinChars && sInput && sInput.length < oSettings.iMinChars) {
				return;
			}
		...
		function _fnAddColumn( oSettings, oOptions, nTh )
		{
			...
			if ( typeof oOptions != 'undefined' && oOptions !== null )
			{
				...
				_fnMap( oCol, oOptions, "sSplitChar" );
				_fnMap( oCol, oOptions, "bNameSearch" );
			}
		...
		function _fnFeatureHtmlFilter ( oSettings )
		{
			...
			jqFilter.keyup( function(e) {
				if(oSettings.iColumnToSort)
				{	
					
					oSettings.aoPreSearchCols[ oSettings.iColumnToSort ].sSearch = $(this).val();
					_fnFilterComplete( oSettings, oSettings.oPreviousSearch, 1 );
		...
		function _fnFilterColumn ( oSettings, sInput, iColumn, bEscapeRegex )
		{
			...
			if (oSettings.aoColumns[iColumn]["sSplitChar"]) {
				sRegexMatch = sRegexMatch.replace(oSettings.aoColumns[iColumn]["sSplitChar"],"|");
			}

			if (oSettings.aoColumns[iColumn]["bNameSearch"]) {
				var posblank = sRegexMatch.indexOf(" ");
				if (posblank > 0) {
					var part1 = sRegexMatch.substring(0, posblank);
					var part2 = $.trim(sRegexMatch.substring(posblank, sRegexMatch.length));
					sRegexMatch = '((\\w*)'+part1+'(\\w*)\\s(\\w*)'+part2+'(\\w*))|((\\w*)'+part2+'(\\w*)\\s(\\w*)'+part1+'(\\w*))'
				}
			}
		...
		function _fnSortAttachListener ( oSettings, nNode, iDataIndex, fnCallback )
		{
			...
			if (oSettings.sPaginationType=="letter_pagination" && oSettings.iColumnToSort==iDataIndex){
				var _aaTemp = new Array();
				
				var _aSort1 = [ oSettings.iColumnToSort, 'asc' ];
				_aaTemp[0] = _aSort1;
				
				if(oSettings.iColumnToSortSecondary)
				{
					var _aSort2 = [ oSettings.iColumnToSortSecondary, 'asc' ];
					_aaTemp[1] = _aSort2;
				}
				//oTable1.fnSort( [ [2,'asc'], [1,'asc'] ] )
				
				oSettings.aaSorting = _aaTemp;
				_fnSort( oSettings );
			
				// trigger click event to sort alphabetically based on iColumnToSort parameter supplied
				//$(nNode).trigger('click');
			}
		}
...
//						"Please use the API to make any configuration changes required." );
			...
			if ( typeof oInit != 'undefined' && oInit !== null )
			{
				...
				_fnMap( oSettings, oInit, "iColumnToSort" );
				_fnMap( oSettings, oInit, "iColumnToSortSecondary" );
				_fnMap( oSettings, oInit, "iLetterPaginationShowPages" );
				...
				// add by Mingbo.Wang 04/25/2012
				_fnMap( oSettings, oInit, "iMinChars");
