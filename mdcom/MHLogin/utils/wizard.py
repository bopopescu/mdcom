...
imported from django 1.2 and fixed up a bit
"""
...
class FormWizard(object):
	...
	def get_form(self, step, data=None):
		...
		return self.form_list[step](data, initial=self.initial.get(step, None))
	...
	def __call__(self, request, *args, **kwargs):
		...
		if current_step >= self.num_steps():
			raise Http404(_('Step %s does not exist') % current_step)

		# For each previous step, verify the hash and process.
		# TODO: Move "hash_%d" to a method to make it configurable.
		for i in range(current_step):
			form = self.get_form(i, request.POST)
			if not form.is_valid() or request.POST.get("hash_%d" % i, '') != self.security_hash(request, form):
				return self.render_hash_failure(request, i)
			try:
				self.process_step(request, form, i)
			except forms.ValidationError as e:
				nonfield_errors = form.errors.get('__all__', [])
				form.errors['__all__'] = form.error_class(nonfield_errors + e.messages)

		# Process the current step. If it's valid, go to the next step or call
		# done(), depending on whether any steps remain.
		if request.method == 'POST':
			form = self.get_form(current_step, request.POST)
		#empty request.GET would cause all fields that require a value to have errors
		elif(current_step == 0 and request.method == 'GET' and request.GET):

			form = self.get_form(current_step, request.GET)
			# Make sure GET variables get saved to the form, for the first step
			# only. Necessary because the generic /register/ view redirects the
			# invitations to the approriate /register/<type>/ view using GET
			# data.
	...
	def determine_step(self, request, *args, **kwargs):
		"""
		Given the request object and whatever \*args and \*\*kwargs were passed
		to __call__(), returns the current step (which is zero-based).
	...
	def render_template(self, request, form, previous_fields, step, context=None):
		...
		#update by xlin in 20120702 to reduce 1
	...
	def done(self, request, form_list):
		...
		data.
		"""
		raise NotImplementedError(_("Your %s class has not defined a done() method, which is required.") % self.__class__.__name__)
