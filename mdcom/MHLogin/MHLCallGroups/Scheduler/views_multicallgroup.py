...
from MHLogin.MHLCallGroups.models import CallGroup, CallGroupMember, \
	CallGroupMemberPending, Specialty
...
@no_cache
def undo(request, practice_id=None, callgroup_id=None):
	return undoOrRedo(request, practice_id, callgroup_id, 
		SessionHelper.SCHEDULE_UNDOSTACK_NAME, SessionHelper.SCHEDULE_REDOSTACK_NAME)


@no_cache
def redo(request, practice_id=None, callgroup_id=None):
	return undoOrRedo(request, practice_id, callgroup_id, 
...
#add by xlin 121122 to find all providers
...
def getPenddings(request):
	#add accept status and 0 is new pending
	mhluser_id = request.user.id
	pend_list = get_call_group_penddings(mhluser_id)
	return HttpResponse(json.dumps(pend_list))

def get_call_group_penddings(mhluser_id):
	pendings = CallGroupMemberPending.objects.filter(to_user__pk=mhluser_id, 
			accept_status=0).order_by('created_time')
	pend_list = [{
		'id': p.id,
		'from_user': ' '.join([p.from_user.first_name,\
				p.from_user.last_name]),
		'to_user': ' '.join([p.to_user.first_name, p.to_user.last_name]),
		'call_group_team': p.call_group.team if p.call_group else ''
	} for p in pendings]
	return pend_list

def joinCallGroup(request, type):
	if request.method == 'POST':
		id = request.POST['id']
		provider = request.session['MHL_Users']['Provider']
		ret_data = join_call_group(id, type, provider)
...
def join_call_group(call_group_id, type, provider):
	...
	if not pend_members or not pend_members[0].call_group:
		...
		ret_data["message"] = _('You already have been added to the call group or declined the invitation from other client.')
		return ret_data
	else:
	...
	if type == 'Accept':
		...
		if (not CallGroupMember.objects.filter(call_group=call_group, \
			...
			CallGroupMember(call_group=call_group, member=provider,
						alt_provider=1).save()
