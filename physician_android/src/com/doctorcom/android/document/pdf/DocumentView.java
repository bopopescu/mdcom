...
import com.doctorcom.android.document.pdf.multitouch.MultiTouchZoom;
...
public class DocumentView extends View implements ZoomListener {
    ...
    private final SparseArray<Page> pagees = new SparseArray<Page>();
    ...
    private void init() {
        ...
        for (int i = 0; i < decodeService.getPageCount(); i++) {
        	pagees.put(i, new Page(this, i));
        	pagees.get(i).setAspectRatio(width, height);
        }
        isInitialized = true;
        invalidatePageSizes();
    ...
    private void goToPageImpl(final int toPage) {
        scrollTo(0, pagees.get(toPage).getTop());
    }
    ...
    private void updatePageVisibility() {
    	for (int i = 0, len = pagees.size(); i < len; i++) {
    		Page page = pagees.valueAt(i);
    ...
    private void lineByLineMoveTo(int direction) {
        if (direction == 1 ? getScrollX() == getRightLimit() : getScrollX() == getLeftLimit()) {
            scroller.startScroll(getScrollX(), getScrollY(), direction * (getLeftLimit() - getRightLimit()), (int) (direction * pagees.get(getCurrentPage()).bounds.height() / 50));
        } else {
    ...
    private int getBottomLimit() {
        return (int) pagees.get(pagees.size() - 1).bounds.bottom - getHeight();
    }
    ...
    void invalidatePageSizes() {
        ...
        for (int i = 0; i < pagees.size(); i++) {
            Page page = pagees.get(i);
