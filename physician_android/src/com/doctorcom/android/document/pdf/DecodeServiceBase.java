...
import com.doctorcom.android.document.pdf.codec.CodecPage;
import com.doctorcom.android.document.pdf.utils.PathFromUri;
...
public class DecodeServiceBase implements DecodeService
{
    ...
    private final SparseArray<SoftReference<CodecPage>> page = new SparseArray<SoftReference<CodecPage>>();
    ...
    {	if (page.get(pageIndex) == null || page.get(pageIndex).get() == null)
        {
            page.put(pageIndex, new SoftReference<CodecPage>(document.getPage(pageIndex)));
            ...
            if (pageEvictionQueue.size() > PAGE_POOL_SIZE) {
                ...
                CodecPage evictedPage = page.get(evictedPageIndex).get();
                page.remove(evictedPageIndex);
                if (evictedPage != null) {
                    evictedPage.recycle();
                }
            }
        }
        return page.get(pageIndex).get();
    }
        ...
        executorService.submit(new Runnable() {
            public void run() {
            	for (int i = 0, len = page.size(); i < len; i++) {
            		SoftReference<CodecPage> codecPageSoftReference = page.valueAt(i);
