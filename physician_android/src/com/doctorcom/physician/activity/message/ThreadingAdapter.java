...
import com.doctorcom.physician.activity.message.ThreadingItem.Body;
import com.doctorcom.physician.net.ImageDownload;
import com.doctorcom.physician.utils.Utils;

public class ThreadingAdapter extends BaseAdapter {
	protected Context context;
	private LayoutInflater mInflater;
	private List<ThreadingItem> mData;
	private int index = 0;
	@SuppressWarnings("unused")
	private AppValues appValues;

	public ThreadingAdapter(Context context) {
	...
	@Override
	public ThreadingItem getItem(int position) {
		if (mData == null) {
			return null;
		}
	...
	public void addItem(ThreadingItem item) {
		if (mData == null) {
			mData = new ArrayList<ThreadingItem>();
		}
		mData.add(item);
	}

	public void addItems(List<ThreadingItem> items) {
		index = this.getCount();
		for (int i = 0, length = items.size(); i < length; i++) {
			ThreadingItem item = items.get(i);
			addItem(item);
		}
	}
	...
	public void setThreadingBody2(ArrayList<Body> list) {
		HashMap<String, ThreadingItem> messageMap = getThreadingMap();
		int length = list.size();
		for (int i = 0; i < length; i++) {
			Body itemBody = list.get(i);
			String id = itemBody.getId();
			ThreadingItem item = messageMap.get(id);
	...
	public HashMap<String, ThreadingItem> getThreadingMap() {
		int size = mData.size();
		HashMap<String, ThreadingItem> threadingMap = new HashMap<String, ThreadingItem>();
		for (int i = 0; i < size; i++) {
			ThreadingItem item = mData.get(i);
	...
	public void addTopItems(List<ThreadingItem> items) {
		if (mData == null) {
			mData = new ArrayList<ThreadingItem>();
		}
		for (int i = items.size(); i > 0; i--) {
			ThreadingItem item = items.get(i - 1);
			mData.add(0, item);
		}
	}

	public void initItems(List<ThreadingItem> items) {
		index = 0;
		if (mData != null) {
			mData.clear();
			mData = null;
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		ThreadingItem item = getItem(position);
		MessageViewHolder holder;
		String senderName = item.getSender().getName();
		AppValues appValues = new AppValues(this.context);
		String currentUserName = appValues.getCurrentUserName();
		...
		final ThreadingItem viewItem = item;
		...
		if (item.isRead()) {
			...
			if (senderName.equals(currentUserName))
				holder.llchatContainer
						.setBackgroundResource(R.drawable.chatto_read);
			else
				holder.llchatContainer
